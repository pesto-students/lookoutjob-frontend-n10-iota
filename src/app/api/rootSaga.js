...
export default function* rootSaga() {
  yield takeLatest(loginActions.fetchLogin.type, loginSaga);
  yield takeLatest(applicationActions.fetchApplication.type, applicationSaga);
  yield takeLatest(userDetailsActions.fetchUserDetails.type, userDetailSaga);
  yield takeLatest(
    userEducationActions.fetchUserEducation.type,
    postEducationSaga
  );
  yield takeLatest(postSkillsActions.fetchUserDetails.type, postSkillsSaga);

  yield takeLatest(postDetailsActions.fetchUserDetails.type, postDetailsSaga);

  yield takeLatest(connectionsActions.fetchConnections.type, connectionSaga);
  yield takeLatest(timelineActions.fetchtimeline.type, timelineSaga);
  yield takeLatest(searchActions.fetchSearch.type, searchSaga);
  yield takeLatest(
    connectionsReqActions.fetchConnectionsReq.type,
    connectionReqSaga
  );
  yield takeLatest(requestActions.fetchRequest.type, requestSaga);
  yield takeLatest(companyActions.fetchcompany.type, companySaga);

  yield takeLatest(createUserActions.fetchCreateUser.type, createUserSaga);

  yield takeLatest(
    companyDetailsActions.fetchcompanyDetails.type,
    companyDetailsSaga
  );

  yield takeLatest(jobActions.fetchjob.type, jobSaga);
  yield takeLatest(shortlistActions.fetchShortlist.type, shortlistSaga);
}
